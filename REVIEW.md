...
#### P0-2 BEP-52 v2 / 混合种子 info_hash 计算错误
...
1. **P0-1 单文件种子路径** — 功能完全失效，改动小
2. **P0-2 v2 info_hash** — 数据标识错误，改动小
3. **初始化 Git 仓库** — 当前成果无版本保护，风险高于其余各项
4. **P1-1 纯 v2 明确提示** — 顺带完成，成本极低

### 第二阶段：可观测性（P1）

5. **P1-2 日志系统 + 高危异常改造**（先改 8 处高危）
6. **P1-3 shutdown 加锁与 join**

### 第三阶段：性能与资源（P2）

7. **P2-1 缓冲区计算优化**（先做消除重复 + 增量扫描）
8. **P2-2 缓存上限与 LRU**

### 第四阶段：质量基建（长期）

9. 测试矩阵补齐（混合 v2 × 两条入口）+ 纯函数单元测试
10. 覆盖率接入 + 统一测试入口
11. 打包分发与依赖锁定
...
1. `.venv_old_broken`（约 830 MB）需用户手动删除——此前 pip 安装被中断导致文件清零，已重命名隔离，当前程序不使用；受安全删除策略限制无法代删。
